...
const VocabularyLessonDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const lessonData = await getVocabularyLessonById(id);
        setLesson(lessonData);
        
        // Increment view count
        fetch(`http://localhost:8080/api/vocabulary-lessons/${id}/view`, {
          method: 'POST',
          headers: {
        ...
        if (lessonData.examId) {
          try {
            const examData = await getExamById(lessonData.examId);
            setExam(examData);
          } catch (examError) {
      ...
      <>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">Bài học này có thể đã bị xóa hoặc chưa được xuất bản.</p>
            <Link 
              to="/vocabulary-lessons" 
              className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
            >
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <article className="bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            {/* Related Exam */}
            {exam && (
              <div className="px-6 py-6 bg-gradient-to-r from-blue-50 to-purple-50 border-t border-gray-200">
